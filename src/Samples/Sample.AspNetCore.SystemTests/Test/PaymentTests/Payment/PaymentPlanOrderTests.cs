        ...
        public void CreateOrderWithPaymentPlanAsPrivateAsync(Product[] products)
        {
            Assert.DoesNotThrowAsync(async () => 
            {
                ...
                .RefreshPageUntil(x =>
                    ...
                    x.Orders.Count() > 0 &&
                    x.Orders.First().GetContent(ContentSource.TextContent).Contains(nameof(OrderStatus.Open)), 30, 5)

                .Orders.First().Order.OrderId.StoreValue(out var orderId)

                // Validate order info
                .Orders.First().Order.OrderStatus.Should.Equal(nameof(OrderStatus.Open))
                .Orders.First().Order.PaymentType.Should.Equal(nameof(PaymentType.PaymentPlan))
